...
`artifact_path` is a nonempty, normalized relative path. It cannot escape storage. Owned filenames are unique bare filenames, excluding `manifest.json` and the reserved temporary-file namespace: `worker.publishing` appends `.{call_id}.tmp` to a filename rather than replacing its extension, so `train.bin` stages through `train.bin.fc-01.tmp` and neither two owned files nor two calls can share one temporary name. This keeps nested artifacts independent:
...
Execution ownership must prevent two workers from **publishing** to the same path; it cannot prevent two from running. Launch decisions are serialized, and ownership is released either by the call itself ending or by a cancel, which drops the lease before it asks the platform to stop the call -- the platform delivers that whenever it next can, so the released lease is the stop that lands first. A second call may therefore be granted the path while the first is still writing. What keeps that safe is where the fence sits: a job confirms the grant inside `worker.publishing`, between writing its temporary file and renaming it into place, and that temporary file carries the call id, so overlapping calls never write the same bytes and only the lease holder publishes. That rename is the only step the worker still controls: the mount commits in the background, so a call that never reaches `volume.commit()` publishes what it wrote anyway. A call that was cancelled or superseded therefore leaves its earlier checkpoints and no finished artifact, and deletes the temporary file it never published. A lease check immediately before publishing still cannot fence off a still-running worker: it can stop a rename from being made, never unmake one, so a call also reports how it ended. Log handling must not serve as an unconditional path for publishing unfinished outputs.
