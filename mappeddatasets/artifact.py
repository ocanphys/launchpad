...
if TYPE_CHECKING:
    from mappeddatasets.tokenstream import TokenStream
...
class MappedDataSet(Artifact):
    ...
    a TokenStream per split (mappeddatasets/tokenstream.py) that serves
    windows straight out of each source's own tokens.bin via memmap.

    A different kind of artifact from datasets.artifact.DataSet, not just a
    variant of it -- one physically copies tokens, this one never writes
    anything, so each lives in its own top-level package the way tokenizers/
    ...
    `Declaration`, and every notebook that already knows how to find an
    artifact by path find this one too. The folder just never holds
    anything a job wrote; the manifest `Declaration.write` puts there is
    the only file in it.
    ...
    def completion_paths(self, root: Path) -> list[Path]:
        ...
        for why the base class needs this hook at all, and dag/resolve.py's
